...
// ── P6: LLM JSON 清洗（与 LianBot clean_json 完全一致）─────────────────────
...
fn parse_llm_topics(content: &str) -> Vec<Topic> {
    ...
    let result = serde_json::from_str::<serde_json::Value>(&cleaned)
        .ok()
        .and_then(|v| {
...
返回 JSON 对象：{{"topics":[{{"topic":"","contributors":[],"detail":""}}]}}"#)
}
...
返回 JSON 对象：{{"relationships":[{{"type":"duo","members":[],"label":"","vibe":"","evidence":[]}}]}}"#)
}
